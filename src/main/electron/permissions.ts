...
export namespace NodeMacPermissions {
    export const hasScreenPremise = () => getAuthStatus('screen');
    export const requestScreenPremise = () => askForScreenCaptureAccess();
}
...
export const ensureScreenCapturePermissions = (fallback = Permissions.screenCaptureFallback) => {

    const hasAccess = hasScreenPremise();
    const hadAsked = true
    // 用 electron 的接口判断
    const electronHasPremise = false // hasScreenPremise();
